...
def create_statistics_report(runs):
    ...
    for run in runs:
        tmp_dic = {'urls': os.path.split(run.get_path())[1] + "/TestReport.html",
                   'name': run.get_device()['model'] + "自动化测试报告"}
...
def backup_report(report_path, backup_path, time_formated):
    """备份旧报告 TestReport文件夹"""
    ...
    if not os.path.exists(report_path):
        ...
        try:
            os.rename(report_path, os.path.join(backup_path, 'Report_' + time_formated))
